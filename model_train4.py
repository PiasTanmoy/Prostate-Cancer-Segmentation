...
# --- Constants ---
NUM_CLASSES = 6  # 0,1,2,3,4,5 (but only 0,2,3,4,5 used)
CLASS_LABELS = [0, 1, 2, 3, 4, 5]  # 1 is unused, but needed for indexing
...
class LazyProstateSegDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mask_slice = mask_volume[z].astype(np.int64)  # int64 for class labels
        ...
        mask_tensor = torch.tensor(mask_slice, dtype=torch.long)  # shape: [H, W], values in {0,2,3,4,5}
...
class UNet(nn.Module):
    def __init__(self, num_classes=NUM_CLASSES):
        ...
        self.out = nn.Conv2d(64, num_classes, 1)  # multi-class output
...
def get_loss_fn(class_weights):
    return nn.CrossEntropyLoss(weight=class_weights, ignore_index=1)  # ignore unused class 1
...
def multiclass_dice_present(pred, target, classes=[0,2,3,4,5], eps=1e-6):
    """
    Computes mean dice only for classes 2-5 that are present in the target mask.
    pred: logits [B, C, H, W]
    target: [B, H, W]
    Returns: mean dice, dict of per-class dice (for present classes)
    """
    pred = torch.argmax(pred, dim=1)  # [B,H,W]
    dices = {}
    present_classes = []
    # Find present classes in target (from 2-5)
    unique_classes = torch.unique(target)
    for cls in classes:
        if (unique_classes == cls).any():
    ...
    for cls in present_classes:
        ...
        dice = (2*inter + eps) / (union + eps)
        dices[cls] = dice.mean().item()
    mean_dice = np.mean(list(dices.values())) if dices else 0.0
    return mean_dice, dices

def multiclass_iou_present(pred, target, classes=[0,2,3,4,5], eps=1e-6):
    """
    Computes mean IoU only for classes 2-5 that are present in the target mask.
    pred: logits [B, C, H, W]
    target: [B, H, W]
    Returns: mean IoU, dict of per-class IoU (for present classes)
    """
    pred = torch.argmax(pred, dim=1)  # [B,H,W]
    ious = {}
    present_classes = []
    ...
    for cls in present_classes:
        ...
        target_cls = (target == cls).float()
        inter = (pred_cls * target_cls).sum(dim=(1,2))
        union = pred_cls.sum(dim=(1,2)) + target_cls.sum(dim=(1,2)) - inter
        iou = (inter + eps) / (union + eps)
        ious[cls] = iou.mean().item()
...
class_weights = compute_class_weights_from_df()  # Use the precomputed weights from CSV
