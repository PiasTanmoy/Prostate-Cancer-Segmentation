...
def multiclass_dice_present(pred, target, classes=[0,2,3,4,5], eps=1e-6):
    """
    Computes mean dice only for classes present in each sample (slice).
    pred: logits [B, C, H, W]
    target: [B, H, W]
    Returns: mean dice over batch, dict of per-class dice (averaged over batch)
    """
    pred = torch.argmax(pred, dim=1)  # [B,H,W]
    ...
    # Average per-class dice over batch
